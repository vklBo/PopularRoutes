...
function initMap(bounds, setStartCoord, setIntermediateCoord, setEndCoord, selectLayer, useMiles) {
    ...
    map.on('moveend', function(ev){ overlays.loadOverlayContent(map);});
    ...
    L.control.layers(tileLayers.getAvailableTileLayers(), overlays.getAvailableOverlays()).addTo(map);
    ...
    overlays.loadOverlayContent(map);
}
